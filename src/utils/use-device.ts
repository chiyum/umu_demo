...
export function useDevice() {
  const MONTHRESHOLD = 768; // 手機裝置的寬度閾值（以像素為單位）

  const isMobile = ref(window.innerWidth <= MONTHRESHOLD); // 判斷是否為手機裝置的標誌

  // 檢查裝置是否為手機，使用 debounce 限制頻繁調用
  const checkDevice = debounce(() => {
    isMobile.value = window.innerWidth <= MONTHRESHOLD; // 當視窗寬度小於等於 768px 時，認為是手機裝置
  }, 200);
