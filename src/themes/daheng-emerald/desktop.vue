...
/** Hero 輪播（3 slide 自動播 5 秒，對齊原稿 setInterval(5000)） */
    ...
    <div class="emer-wrap">
      ...
      <section class="emer-hero">
        <div class="emer-hero__framebox">
          <div
            class="emer-hero__slides"
            :style="{ transform: `translateX(-${heroIdx * 100}%)` }"
          >
            <div
              v-for="(slide, i) in HERO_SLIDES"
              :key="i"
              class="emer-hero__slide"
              :class="
                slide.kind === 'grad'
                  ? [
                      'emer-hero__slide--grad',
                      `emer-hero__slide--${slide.variant}`
                    ]
                  : []
              "
            >
              ...
              />
              <template v-else>
                <span class="emer-hero__kicker">{{ slide.kicker }}</span>
                <h2 class="emer-hero__title">
                  <template v-for="(line, li) in slide.titleLines" :key="li">
                    {{ line }}<br v-if="li < slide.titleLines.length - 1" />
                  </template>
                </h2>
                <span v-if="slide.descStyle === 'rule'" class="emer-hero__rule">
                  {{ slide.desc }}
                </span>
                <p v-else class="emer-hero__desc">{{ slide.desc }}</p>
              </template>
            </div>
          </div>
          <div class="emer-hero__dots">
            <button
              ...
              :key="i"
              type="button"
              class="emer-hero__dot"
              :class="{ 'emer-hero__dot--on': heroIdx === i }"
