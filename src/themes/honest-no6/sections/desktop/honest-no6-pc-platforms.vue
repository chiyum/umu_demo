...
<template>
  <section class="honest-no6-pc-platforms">
    <div class="honest-no6-pc-platforms__tabs">
      <button
        v-for="tab in TABS"
        :key="tab.key"
        type="button"
        class="honest-no6-pc-platforms__tab"
        :class="{
          'honest-no6-pc-platforms__tab--active': activeTab === tab.key
        }"
    ...
    <div class="honest-no6-pc-platforms__grid">
      <div
        v-for="p in currentPlatforms()"
        :key="p.key"
        class="honest-no6-pc-platforms__card"
      >
        <img :src="p.img" :alt="p.label" class="honest-no6-pc-platforms__img" />
        <div class="honest-no6-pc-platforms__title">{{ p.label }}</div>
      </div>
    </div>
  </section>
</template>
...
.honest-no6-pc-platforms {
  margin: 32px 0;
}

.honest-no6-pc-platforms__tabs {
  display: flex;
  flex-wrap: wrap;
  gap: 10px;
  margin-bottom: 16px;
...
.honest-no6-pc-platforms__grid {
  display: grid;
  grid-template-columns: repeat(3, 1fr);
  gap: 16px;
}

.honest-no6-pc-platforms__card {
  display: flex;
...
.honest-no6-pc-platforms__title {
  font-size: 14px;
  font-weight: 600;
