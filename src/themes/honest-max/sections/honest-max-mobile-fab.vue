...
/**
 * honest-max mobile Fab「切換版本」拖曳球 — 1:1 對齊 honest_real src/widgets/fab.vue
 *
 * 與 honest-at / honest-no6 的 Fab 相同元件（原作三 page 共用），本 theme 內各自複製一份
 * 保 BEM scope 純淨
 */
...
<template>
  ...
  >
    <div>切換版本</div>
    <div v-if="isShowActions" class="honest-max-m-fab__actions">
      <div
        v-for="item in MODE_LIST"
        :key="item.value"
        class="honest-max-m-fab__action-item"
        @click.stop="changeMode(item)"
      >
        <span class="honest-max-m-fab__action-icon" aria-hidden="true">▶</span>
        <div>{{ item.title }}</div>
      </div>
    </div>
  </div>
