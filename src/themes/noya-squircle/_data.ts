/**
 * noya-squircle（諾亞 Squircle 漸層磚）共用資料
 *
 * 移植自設計稿 ly9：Squircle 大圓角方磚 + 玻璃高光卡
 * 類別限定 live（真人視訊），卡片採三層合成結構
 * assets 沿用 noya-blue 切片（hero / DG / allbet）
 */
...
/**
 * 各分類遊戲卡（對齊設計稿 GAMES，三層合成卡）
 *
 * - live：使用真實切片 img
 * - 其他分類：使用三層合成結構（g-bg + g-txt + g-fig）
 */
...
export const LOBBY_CARDS: Record<string, LobbyCard[]> = {
  live: [
    ...
    { name: "WM 真人", en: "WM LIVE", img: cardDgSrc, alt: "WM真人 WM LIVE" }
  ],
  sport: [
